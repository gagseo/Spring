...
public class MyAspect {
	...
	// 메서드 실행 전
	@Before("execution(public * com.test05.*.* (..))")
	...
	// @After
	@After("execution(public * com.test05.*.* (..))")
	public void after(JoinPoint join) {
		System.out.println("집에간다.");
	}

	// 예외를 던지고 난 후
	@AfterThrowing("execution(public * com.test05.*.* (..))")
	public void throwing(JoinPoint join) {
		System.out.println("쉬는 날 이였다.");
	}

	// 만약에 리턴 값이 있다면, 리턴을 하고나서, 리턴값을 받아서 사용할 수 있다.
	@AfterReturning(pointcut = "execution(public * com.test05.*.* (..))", returning = "returnVal")
